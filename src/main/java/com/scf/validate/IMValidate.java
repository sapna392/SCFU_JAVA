...
public class IMValidate {
	...
	public static String validateIMData(IM imModel) {
		String   validateMessage       	 	= "";
		String[] imMobileNumbersArr     = null;
		try {
			if(imModel.getName() == null || imModel.getName().isEmpty() ||imModel.getName().equals("") ||imModel.getName().length() >30) {
				validateMessage ="IM Name Cannot be empty or length cannot be greater than 30";
				return validateMessage;
			}
			if(imModel.getNickName()== null ||imModel.getNickName().isEmpty() || imModel.getNickName().equals("") || imModel.getNickName().length() >30) {
